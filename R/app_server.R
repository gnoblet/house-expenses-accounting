...
app_server <- function(input, output, session) {
  ...
  mod_results_server("results", 
    calculations = reactive({ shared_values$results }),
    start_date = reactive({ 
      if(is.null(setup_data$start_date())) Sys.Date() else setup_data$start_date() 
    }),
    end_date = reactive({ 
      if(is.null(setup_data$end_date())) Sys.Date() else setup_data$end_date() 
    }),
    absences_data = reactive({ setup_data$absences_data() }),
    exceptions_data = reactive({ setup_data$exceptions_data() })
  )
  
  # Long-term analysis module - pass reactive values
  mod_long_term_server("long_term",
    expenses_data = reactive({ setup_data$expenses_data() })
  )
}
